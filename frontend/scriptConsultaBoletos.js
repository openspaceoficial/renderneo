...
async function carregarBoletos(status = 'todos') {
    ...
    const url =
        status === 'todos'
            ? 'http://localhost:5002/api/boletos/todos'
            : `http://localhost:5002/api/boletos/todos?status=${encodeURIComponent(status)}`;
...
async function excluirBoleto(id) {
    ...
    try {
        const response = await fetch(`http://localhost:5002/api/boletos/${id}`, {
            method: 'DELETE'
        });
...
if (!nome_pagador || !cpf_pagador || !endereco_pagador || !valor || !status || !dataEmissao || !dataVencimento || !descricao) {
    ...
    try {
        const response = await fetch(`http://localhost:5002/api/boletos/${boletoId}`, {
            method: 'PUT',
            headers: {
