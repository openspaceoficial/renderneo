...
const allowedOrigins = [
    'undefined',
    'https://3d1b-2804-53e0-823a-4000-4570-8156-5eb-5ed4.ngrok-free.app',
    'http://127.0.0.1:5500',
    'http://localhost:5002',
    'https://sistemaneobh.com.br',
    'https://da8733f42d8e8e0b6e9ee056b7206a8b.serveo.net',
    'https://3d1b-2804-53e0-823a-4000-4570-8156-5eb-5ed4.ngrok-free.app',
    'https://603f-2804-53e0-8200-f400-6013-5aa5-1d2e-6bc8.ngrok-free.app'
];
app.use(cors({
    origin: ['*', 'https://3d1b-2804-53e0-823a-4000-4570-8156-5eb-5ed4.ngrok-free.app'], // Permite todas as origens
...
app.get('/api/cheques/beneficiario', async (req, res) => {
    const { nome } = req.query; // Pega o filtro de status enviado via query string
    let sql = 'SELECT * FROM cheques';

    // Adiciona o filtro de status se estiver presente
    if (nome) {
        sql += ' WHERE nome_beneficiario LIKE ?';
    }

    try {
        const [results] = await db.query(sql, [nome]); // Executa com ou sem o filtro
        if (results.length > 0) {
            res.json(results);
        } else {
            res.status(404).json({ message: 'Nenhum cheque encontrado.' });
...
app.get('/api/cheques/relatorio', async (req, res) => {
    ...
    // Formatação das datas de início e fim para o formato 'YYYY-MM-DD HH:mm:ss'
    let formattedDataInicio = new Date(dataInicio);
    let formattedDataFim = new Date(dataFim);

    // Garantir que a data de início seja no formato 'YYYY-MM-DD 00:00:00'
    formattedDataInicio = new Date(formattedDataInicio.setHours(0, 0, 0, 0)).toISOString().slice(0, 19).replace('T', ' ');

    // Garantir que a data de fim seja no formato 'YYYY-MM-DD 23:59:59'
    formattedDataFim = new Date(formattedDataFim.setHours(23, 59, 59, 999)).toISOString().slice(0, 19).replace('T', ' ');

    const sql = `
        SELECT *
        FROM cheques
        WHERE data_vencimento BETWEEN ? AND ?
        ORDER BY data_vencimento ASC  -- Ordenação por data de vencimento do menor para o maior
    `;
    const values = [formattedDataInicio, formattedDataFim];
