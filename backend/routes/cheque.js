...
router.get('/api/cheques', (req, res) => {
    ...
    //Adiciona filtro de nome, se necessário
    if (nome) {
...
router.get('/api/cheques/beneficiario', (req, res) => {
    const nome = req.query.nome; // Obtém o valor do filtro "nome"
    let sql = 'SELECT * FROM cheques'; // Consulta base
    const params = [];
    ...
    db.query(sql, params, (err, results) => {
        if (err) {
            console.error('Erro ao buscar cheques por beneficiário:', err);
            return res.status(500).json({ error: 'Erro ao buscar cheques.' });
        }
